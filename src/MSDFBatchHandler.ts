...
const SimpleFragmentShader = [
    ...
    '',
    '    // "This quad is a soft shadow." Per-corner and interpolated, like every',
    '    // other channel, so it may only ever weight a blend between two behaviours',
    '    // that agree at its ends — never select how another channel is decoded.',
    ...
    '    float fade = max(smoothstep(0.0, 0.2, outlineDist), softStep);',
    '',
    ...
    '    // zero, which is every shadow quad and every layered outline silhouette —',
    '    // has no use for inColor, so it carries a second colour there instead. The',
    '    // outline / shadow layer then ramps from outOutline.rgb at its outer edge',
    '    // to that inner colour where it meets the glyph: a white-hot glow core',
    '    // inside a coloured halo, or a neon-tube outline. A live fill owns inColor,',
    '    // so the gate switches the ramp off and the composite below is unchanged.',
    '    //',
    '    // `tone` is depth into the layer\'s own visible body, 0 at the outer end',
    '    // and 1 where it meets the glyph. The renderer never gives one quad both a',
    '    // width and a softness, so `band` picks whichever is live:',
    ...
    '    // With neither (a hard shadow) it collapses onto the 1-pixel AA edge,',
    '    // which is all the body such a silhouette has.',
    '    float halfSoft = 0.5 * softNorm;',
    '    float band = max(widthNorm, halfSoft);',
    '    float tone = clamp((outlineDist - outlineEdge + halfSoft) / max(band, 1.0 / px), 0.0, 1.0);',
    '',
