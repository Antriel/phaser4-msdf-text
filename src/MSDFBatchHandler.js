...
const SimpleFragmentShader = [
    ...
    '    ',
    '    // Screen-space slope of the SDF, used to build a ~1px-wide AA ramp',
    '    // around any threshold. Auto-scales with pxRange and zoom level.',
    '    float w = max(fwidth(dist), 0.0001);',
    '    ',
    '    if (uOutlineWidth > 0.0)',
    '    {',
    '        // outlineEdge < textEdge: outline occupies the SDF band between them.',
    '        // uOutlineWidth is in distance-field units; divide by uPxRange to map to [0,1] SDF space.',
    '        float textEdge = 0.5;',
    '        float outlineEdge = 0.5 - (uOutlineWidth / uPxRange);',
    '        ',
    '        float textAlpha = clamp((dist - textEdge) / w + 0.5, 0.0, 1.0);',
    '        float outsideOutline = clamp((dist - outlineEdge) / w + 0.5, 0.0, 1.0);',
    '        float outsideText = 1.0 - textAlpha;',
    '        ',
    '        // Fade outline out in deep background to prevent halo at very negative distances',
